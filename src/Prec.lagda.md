...
+≤+  : {M : Γ ⊢ ⟨ E ⟩ A} {M′ : Γ′ ⊢ ⟨ E′ ⟩ A′} {Γ≤ : Γ ≤ᴳ Γ′}
  ...
  →  Γ≤ ⊢ M ≤ᴹ M′ ⦂ ⟨ e ⟩ s
     ------------------------------------
  →  Γ≤ ⊢ cast (+ p) M ≤ᴹ cast (+ q) M′ ⦂ ⟨ e ⟩ t
+≤+ {p = p} {t = t} M≤M′ = cast≤ {a = p ⨟ t} (≤cast M≤M′)
```
...
-≤- : {Γ≤ : Γ ≤ᴳ Γ′}
  ...
  → Γ≤ ⊢ M ≤ᴹ M′ ⦂ ⟨ e ⟩ t
    --------------------------------------
  → Γ≤ ⊢ cast (- p) M ≤ᴹ cast (- q) M′ ⦂ ⟨ e ⟩ s
-≤- {p = p} {t = t} M≤M′ = ≤cast {a = p ⨟ t} (cast≤ M≤M′)
```

```
infix 3 _≤ᵖ_
data _≤ᵖ_ {E E′ F F′} : E =>ᵉ F → E′ =>ᵉ F′ → Set where
  +≤ᵖ+ : ∀ p q → + p ≤ᵖ + q
  -≤ᵖ- : ∀ p q → - p ≤ᵖ - q
```

```
refl-≤ᵖ : (±p : E =>ᵉ F) → ±p ≤ᵖ ±p
refl-≤ᵖ (+ p) = +≤ᵖ+ p p
refl-≤ᵖ (- p) = -≤ᵖ- p p
```
...
castᵉ≤castᵉ : {Γ≤ : Γ ≤ᴳ Γ′}
  ...
  → Γ≤ ⊢ M ≤ᴹ M′ ⦂ ⟨ s ⟩ a
    --------------------------------------
  → Γ≤ ⊢ castᵉ ±p M ≤ᴹ castᵉ ±q M′ ⦂ ⟨ t ⟩ a
castᵉ≤castᵉ {t = t} (+≤ᵖ+ p q) M≤M′ = castᵉ≤ {e = p ⨟ᵉ t} (≤castᵉ M≤M′)
castᵉ≤castᵉ {s = s} (-≤ᵖ- p q) M≤M′ = ≤castᵉ {e = p ⨟ᵉ s} (castᵉ≤ M≤M′)
```
...
reflᴹ (cast (- p) M)  =  -≤- (reflᴹ M)
reflᴹ (castᵉ ±p M)    =  castᵉ≤castᵉ (refl-≤ᵖ ±p) (reflᴹ M)
reflᴹ blame           =  blame≤
...
ren≤ ρ≤ (⇑≤⇑ g M≤)           =  ⇑≤⇑ g (ren≤ ρ≤ M≤)
ren≤ ρ≤ (≤⇑ g M≤)            =  ≤⇑ g (ren≤ ρ≤ M≤)
ren≤ ρ≤ (cast≤ M≤)           =  cast≤ (ren≤ ρ≤ M≤)
ren≤ ρ≤ (≤cast M≤)           =  ≤cast (ren≤ ρ≤ M≤)
ren≤ ρ≤ (castᵉ≤ M≤)           =  castᵉ≤ (ren≤ ρ≤ M≤)
ren≤ ρ≤ (≤castᵉ M≤)           =  ≤castᵉ (ren≤ ρ≤ M≤)
ren≤ ρ≤ blame≤               =  blame≤
ren≤ {ρ = ρ} ρ≤ {E = E} (wrap≤ {N = N} {∓s = ∓s} {±t} {±e} ƛN≤ƛN′)
...
sub≤ σ≤ (⇑≤⇑ g M≤)           =  ⇑≤⇑ g (sub≤ σ≤ M≤)
sub≤ σ≤ (≤⇑ g M≤)            =  ≤⇑ g (sub≤ σ≤ M≤)
sub≤ σ≤ (cast≤ M≤)           =  cast≤ (sub≤ σ≤ M≤)
sub≤ σ≤ (≤cast M≤)           =  ≤cast (sub≤ σ≤ M≤)
sub≤ σ≤ (castᵉ≤ M≤)          =  castᵉ≤ (sub≤ σ≤ M≤)
sub≤ σ≤ (≤castᵉ M≤)          =  ≤castᵉ (sub≤ σ≤ M≤)
sub≤ σ≤ blame≤               =  blame≤
sub≤ {σ = σ} σ≤ {E = E} (wrap≤ {N = N} {∓s = ∓s} {±t} {±e} ƛN≤ƛN′)
...
sub≤ σ≤ (handle≤handle H≤ M≤) = handle≤handle sub≤ʰ (sub≤ σ≤ M≤)
  where
    ...
    sub≤-on-perform [] = []
    sub≤-on-perform ((refl , B⇒Q≤ , M≤) ∷ Ms≤)
      = (refl , B⇒Q≤ , sub≤ (sub▷≤ (sub▷≤ σ≤)) M≤) ∷ sub≤-on-perform Ms≤
...
data _⊢_⇒ᶠ_∋_≤_ {Γ Γ′} (Γ≤ : Γ ≤ᴳ Γ′)
  ...
  [_]·_ : ∀ {ℰ ℰ′} {M M′}
      {a : A ⇒ ⟨ E ⟩ B ≤ A′ ⇒ ⟨ E′ ⟩ B′}
    → (ℰ≤ : Γ≤ ⊢ P≤ ⇒ᶠ ⟨ eff a ⟩ a ∋ ℰ ≤ ℰ′)
    → (M≤ : Γ≤ ⊢ M ≤ᴹ M′ ⦂ ⟨ eff a ⟩ dom a)
      ----------------------------------------------
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ eff a ⟩ cod a ∋ [ ℰ ]· M ≤ [ ℰ′ ]· M′
  ...
  cast≤ : ∀ {ℰ ℰ′} {±p : A => B}
    ...
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ a ∋ ℰ ≤ ℰ′
      --------------------------------------
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ b ∋ `cast ±p [ ℰ ] ≤ ℰ′

  ≤cast : ∀ {ℰ ℰ′} {e : E ≤ᵉ E′}
            {±p : A′ => B′} {a : A ≤ A′} {b : A ≤ B′}
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ a ∋ ℰ ≤ ℰ′
      --------------------------------------
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ b ∋ ℰ ≤ `cast ±p [ ℰ′ ]

  castᵉ≤ : ∀ {ℰ ℰ′} {±p : E =>ᵉ F} {e : E ≤ᵉ E′} {f : F ≤ᵉ E′} {a : A ≤ A′}
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ a ∋ ℰ ≤ ℰ′
      --------------------------------------
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ f ⟩ a ∋ `castᵉ ±p [ ℰ ] ≤ ℰ′

  ≤castᵉ : ∀ {ℰ ℰ′} {e : E ≤ᵉ E′} {f : E ≤ᵉ F′} {±p : E′ =>ᵉ F′} {a : A ≤ A′}
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ e ⟩ a ∋ ℰ ≤ ℰ′
      --------------------------------------
    → Γ≤ ⊢ P≤ ⇒ᶠ ⟨ f ⟩ a ∋ ℰ ≤ `castᵉ ±p [ ℰ′ ]
...
gvalue≤gvalue (ƛ _) (ƛ _) (wrap≤ ƛN≤ƛN′) = wrap≤ ƛN≤ƛN′
gvalue≤gvalue (ƛ _) (ƛ _) (≤wrap ƛN≤ƛN′) = ≤wrap ƛN≤ƛN′
```
\fi
...
≤gvalue (ƛ _) (ƛ _) (wrap≤ ƛN≤ƛN′) = wrap≤ ƛN≤ƛN′
≤gvalue (ƛ _) (ƛ _) (≤wrap ƛN≤ƛN′) = ≤wrap ƛN≤ƛN′
```
\fi
